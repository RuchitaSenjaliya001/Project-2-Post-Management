...
export default function Input(props) {
    const { title, type, value, onChange, name, errorMsg, className, onBlur, isTouched } = props
    return (
        <div className={`my-5 ${className}`}>
            <div className="relative">
                <input id={name} type={type} value={value} name={name}
                    ...
                    placeholder="" onChange={onChange} onBlur={onBlur} />
                <label htmlFor={name}
            ...
            {errorMsg && isTouched ? <p className="text-red-500 text-sm">{errorMsg}</p> : null}
